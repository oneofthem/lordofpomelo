...
Handler.prototype.inviteJoinTeam = function (msg, session, next)
{
    ...
    this.app.rpc.manager.teamRemote.inviteJoinTeam(session, args, function (err, ret)
    {
        ...
        if (result === consts.TEAM.OK)
        {
            ...
            messageService.pushMessageToPlayer({uid: inviteeObj.userId, sid: inviteeObj.serverId}, 'onInviteJoinTeam', captainInfo);
        }
    });
    next();
};
...
{
    console.log("inviteJoinTeamReply");
    var area = session.area;
    ...
    if (msg.reply === consts.TEAM.JOIN_TEAM_REPLY.ACCEPT)
    {
        ...
        this.app.rpc.manager.teamRemote.acceptInviteJoinTeam(session, args, function (err, ret)
        {
            ...
            if (result === consts.TEAM.JOIN_TEAM_RET_CODE.OK)
            {
                ...
                {
                    result = consts.TEAM.JOIN_TEAM_RET_CODE.SYS_ERROR;
                    messageService.pushMessageToPlayer({uid: captainObj.userId, sid: captainObj.serverId}, 'onInviteJoinTeamReply', {reply: result});
                } else
                {
...
{
    console.log("applyJoinTeamReply");
    var area = session.area;
    var playerId = session.get('playerId');
    var player = area.getPlayer(playerId);
    ...
    if (msg.reply === consts.TEAM.JOIN_TEAM_REPLY.ACCEPT)
    {
        ...
        this.app.rpc.manager.teamRemote.acceptApplicantJoinTeam(session, args, function (err, ret)
        {
            ...
            if (result === consts.TEAM.JOIN_TEAM_RET_CODE.OK)
            {
                ...
                {
                    result = consts.TEAM.JOIN_TEAM_RET_CODE.SYS_ERROR;
                    messageService.pushMessageToPlayer({uid: applicantObj.userId, sid: applicantObj.serverId}, 'onApplyJoinTeamReply', {reply: result});
                } else
                {
            ...
            {
                messageService.pushMessageToPlayer({uid: applicantObj.userId, sid: applicantObj.serverId}, 'onApplyJoinTeamReply', {reply: ret.result});
            }
        });
    } else
    {
        // push tmpMsg to the applicant that the captain rejected
        messageService.pushMessageToPlayer({uid: applicantObj.userId, sid: applicantObj.serverId}, 'onApplyJoinTeamReply', {reply: consts.TEAM.JOIN_TEAM_REPLY.REJECT});
    }
    next();
};
